...
# SF_PASSWORD = os.getenv('SNOWFLAKE_PAT')
SF_TOKEN = os.getenv('SNOWFLAKE_PAT')
SF_ACCOUNT = os.getenv('SNOWFLAKE_ACCOUNT')
SF_WAREHOUSE = os.getenv('SNOWFLAKE_WAREHOUSE')
SF_DATABASE = os.getenv('SNOWFLAKE_DATABASE')
SF_SCHEMA = os.getenv('SNOWFLAKE_SCHEMA_RAW')
...
def get_latest_api_date():
    ...
    if getattr(response, 'status_code', 0) == 200:
        ...
        if records:
            ...
            latest_date_obj = datetime.strptime(latest_date_str, '%Y-%m-%d') - timedelta(days=1)
            print(f"API's latest full available data is from: {latest_date_obj}")
            return (latest_date_obj).strftime('%Y-%m-%d') # Return the latest date - 1 day
...
def get_date_range(cursor):
    ...
    query = """
        ...
        WHERE http_status_code = 200 AND record_count > 0
    """
